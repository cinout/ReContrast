...
def binary_refinement(
    ...
    pairwise_less = [
        init_queries[i] < init_queries[i + 1] for i in range(len(init_queries) - 1)
    ]
    ...
    for distance, (query_left, query_right) in candidates:
        ...
        interval_queries = np.linspace(
            query_left, query_right, num=2 + num_interval_queries
        )[1:-1]
...
class ThresholdMetrics:
    ...
    def __init__(
        ...
        assert np.array_equal(np.sort(anomaly_thresholds)[::-1], anomaly_thresholds)

        for per_threshold_array in (
            flatten_2d(spros_of_defects_of_images)
    ...
    def merge_with(self, other: "ThresholdMetrics"):
        ...
        anomaly_thresholds = np.concatenate(
            [self.anomaly_thresholds, other.anomaly_thresholds]
        )
        # Merge the nested 1-D numpy arrays.
        fp_areas_per_image = concat_nested_arrays(
            self.fp_areas_per_image, other.fp_areas_per_image
        )
        tn_areas_per_image = concat_nested_arrays(
            self.tn_areas_per_image, other.tn_areas_per_image
        )
        ...
        fp_areas_per_image = get_sorted_nested_arrays(
            nested_arrays=fp_areas_per_image, sort_indices=sort_indices
        )
        tn_areas_per_image = get_sorted_nested_arrays(
            nested_arrays=tn_areas_per_image, sort_indices=sort_indices
        )
    ...
    def reduce_to_images(self, take_anomaly_maps: Sequence[AnomalyMap]):
        ...
        """
        take_indices = [self.anomaly_maps.index(image) for image in take_anomaly_maps]
        return ThresholdMetrics(
            gt_maps=take(self.gt_maps, take_indices),
            anomaly_maps=take(self.anomaly_maps, take_indices),
            anomaly_thresholds=self.anomaly_thresholds,
            spros_of_defects_of_images=take(
                self.spros_of_defects_of_images, take_indices
            ),
    ...
    def get_fp_rates(self):
        ...
        """
        return get_fp_rates(
            fp_areas_per_image=self.fp_areas_per_image,
            tn_areas_per_image=self.tn_areas_per_image,
        )

    def get_mean_spros(self, filter_defect_names: Optional[Iterable[str]] = None):
        ...
        for gt_image, image_spros in zip(self.gt_maps, self.spros_of_defects_of_images):
            if gt_image is None:
            ...
            for gt_channel, spros in zip(gt_image.channels, image_spros):
                ...
                if filter_defect_names is None or defect_name in filter_defect_names:
    ...
    def get_per_image_results_dicts(self, auc_max_fprs: Iterable[float]):
        ...
        for anomaly_map, fp_areas, tn_areas, spros_of_defects in zip(
            ...
            if np.sum(fp_areas + tn_areas) > 0:
                fp_rates = get_fp_rates(
                    fp_areas_per_image=[fp_areas], tn_areas_per_image=[tn_areas]
                )
...
class MetricsAggregator:
    ...
    def _refinement_callback(self, anomaly_thresholds: Sequence):
        ...
        fp_areas_per_image, tn_areas_per_image = get_fp_tn_areas_per_image(
            gt_maps=self.gt_maps,
            anomaly_maps=self.anomaly_maps,
            anomaly_thresholds=anomaly_thresholds_sorted,
            parallel_workers=self.parallel_workers,
        ...
        result_values_sorted = [
            np.array([spro, fp_rate]) for spro, fp_rate in zip(mean_spros, fp_rates)
        ]
    ...
    def _get_initial_thresholds(self, num_thresholds=50, epsilon=1e-6):
        ...
        # FIXME: this is the key area for figuring out if sPRO is calculated image-wise or all-together
        ...
        for anomaly_map in self.anomaly_maps:
            ...
            sampled_scores_image = np.random.choice(
                flat_scores, size=num_sampled_per_image, replace=False
            )
        ...
        equidistant_indices = np.linspace(
            0, len(sampled_scores) - 1, num=num_thresholds, endpoint=True, dtype=int
        )
